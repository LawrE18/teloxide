...
// N.B. we [de]serialize `CustomCustomQueryId` as `{"custom_query_id":n}`, which means that if
//      you want just an integer, you need to special case it with something
//      like `serde(with = "crate::types::option_msg_id_as_int")]`
//
//      (we can't change the default format of `CustomCustomQueryId` because it's returned
